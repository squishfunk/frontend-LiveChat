<template>
  <div class="p-3">
    ...
    <Toolbar class="mt-3 w-full">
      <template #start>
        <div class="mt-0">
          <p class="text-sm text-white mb-0 sm:mt-0">Twój nick:</p>
          <div v-if="hasStoredUsername">
    ...
    <template #container="{ closeCallback }">
      <div class="flex flex-column px-7 py-7 gap-4"
        ...
        <div class="inline-flex flex-column gap-2">
          <label for="username" class="text-primary-50 font-semibold">Twój nick</label>
          <InputText id="username" v-model="username" class="bg-white-alpha-20 border-none p-3 text-primary-50" />
        </div>

        <div class="flex align-items-center gap-2">
          <Button label="Zacznij czatować!" @click="loginCallback(closeCallback)" text
...
const socket = new WebSocket(`ws://${wsUrl}:${port}/`);
console.log(socket);
...
const loginCallback = (closeCallback: () => void) => {
  if (!username.value) {
    toast.add({ severity: 'warn', summary: 'Brakujące dane', detail: 'Podaj nazwę użytkownika', life: 3000 });
    return;
  } else if (username.value.length > 10) {
    toast.add({ severity: 'warn', summary: 'Za długa nazwa użytkownika', detail: 'Nazwa użytkownika nie może mieć wiecej niż 10 znaków', life: 3000 });
    return;
  }
